...
class MyBot:
	...
	for x in range(0, self.level_size[0]):
	  ...
	  if ants.visible(point):
	    self.tiles_seen[y][x] = True
	  if not self.tiles_seen[y][x]:
	    self.unseen_tiles.append((y, x))
      self.percentage_unseen = float(len(self.unseen_tiles)) / float(self.total_tiles)
    ...
    def assign_defense(self):
      ...
      defense_guide, num_dest = pathfind.create_defensive_guide(self.attack_radius - 1, self.my_hills ,self.level_array)
      if ant_hill_ratio >=4 and ant_hill_ratio <= 7:
	ant_num = 1 * num_hills
      elif ant_hill_ratio > 7:
	ant_num = min(self.total_ants / 3, num_dest) * num_hills
      else: ant_num = 0
      if ant_num > 0:
    ...
    def assign_unseen(self):
      ...
      if num_ants_no_guide > 0 and self.unseen_tiles > 0:
	unseen_guide = pathfind.create_guide(self.unseen_tiles, self.level_array)
	...
	for x in range(0, self.level_size[0]):
	  if y % distance == 0 and x % distance == 0:
	    if level[y][x] != '%' and self.tiles_seen[y][x] and self.away_hill((y, x), distance) :
    ...
    def assign_outposts(self, ants):
      ...
      if ant_num > 0:
	destinations = self.get_all_outposts(int(self.view_radius - 1), self.level_array, ants)[0:ant_num]
    ...
    def assign_orders(self, ants):
      ...
      self.move(ants, self.assign_food())
      self.move(ants, self.assign_defense())
      self.move(ants, self.assign_hills())   
      self.move(ants, self.assign_unseen())
      self.move(ants, self.assign_outposts(ants))
